...
업로더는 콜백 수신 서버에서 **5xx 응답 시 3회 지수 백오프 재시도**를 수행합니다(§5.2 참조). 따라서 동일 `batchId` 의 콜백이 중복 도달할 수 있으며, 벤더 서버는 멱등하게 처리해야 합니다.

권장 구현 패턴:

- **기본 방어선**: INTEGRATION.md §5 패턴에 따라 콜백 토큰을 atomic 하게 1회용 consume 하면, 두 번째 요청은 `401` 로 자연 차단됩니다.
- **재시도 엣지 케이스**: 첫 요청 처리 도중 서버가 `5xx` 를 반환한 경우, 토큰이 이미 consume 되었어도 업로더가 재시도합니다. 따라서 서버는 `(batchId, status, hiraReceiptNo)` 를 **upsert** 방식으로 처리하는 것을 권장합니다.
...
**하위호환 폴백**: `CANCELLED` 를 인식하지 못하는 v1.0 서버가 400 을 반환하면, 업로더는 `FAILED` 로 폴백하여 콜백을 재전송합니다. 단, 배치 상태가 원복(`PENDING`)되지 않아 재시도 UX 가 불편해지므로 서버 업데이트를 권장합니다.
