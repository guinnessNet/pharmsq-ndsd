...
- **HIRA 포털 SSL 접속 오류 수정** — 일부 PC에서 로그인 테스트/업로드 시 `ERR_FAILED (-2)` 로 접속이 실패하던 문제 해결. 포털 서버가 중간 인증서(CA)를 함께 보내지 않아 발생하던 인증서 검증 실패를, 누락된 중간 CA를 앱에 번들로 동봉해 보완. 브라우저로 포털을 미리 방문하지 않아도 정상 동작.
